...
describe('Knowledge Graph End-to-End Workflows', () => {
  ...
  beforeAll(async () => {
    ...
    // Create sample ADRs
    await fs.writeFile(
      path.join(adrDir, '001-database-choice.md'),
...
`
    ...
    await fs.writeFile(
      path.join(adrDir, '002-api-design.md'),
...
`;
      ...
      const updatedTodoContent = todoContent
        .replace('- [ ] test-jwt-middleware', '- [x] test-jwt-middleware')
        .replace('- [ ] mock-jwt-service', '- [x] mock-jwt-service');
      ...
      await kgManager.addToolExecution(
        ...
        },
        {
          ...
          recommendations: ['Add more edge case tests', 'Improve error handling documentation'],
        },
        true,
    ...
    test('should handle complex multi-developer workflow', async () => {
...
`;
      ...
      const allTasksCreated = snapshot.intents.flatMap((intent: any) => intent.todoTasksCreated);
      ...
      const sharedTaskModifications = snapshot.intents.flatMap(
        (intent: any) => intent.todoTasksModified
      );
  ...
  describe('Performance Under Load', () => {
    ...
    test('should scale with large TODO.md files', async () => {
      ...
      const largeTodoLines = Array.from(
        { length: 2000 },
        (_, i) => `- [ ] large-task-${i}: Description for task ${i}`
      );
