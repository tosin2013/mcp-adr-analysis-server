...
// Create mock functions that will be used by all instances
const mockSetConfidenceThreshold = jest.fn();
const mockAnswerResearchQuestion = jest.fn();

// Mock the ResearchOrchestrator class
class MockResearchOrchestrator {
  constructor(_projectPath: string, _adrDirectory: string) {
...
// Setup the module mock using jest.unstable_mockModule (for ES modules)
...
// IMPORTANT: These imports must come AFTER jest.unstable_mockModule
const { performResearch } = await import('../../src/tools/perform-research-tool.js');
const { McpAdrError } = await import('../../src/types/index.js');

describe('Perform Research Tool', () => {
  beforeEach(() => {
    // Clear only the call history, not the implementation
    mockSetConfidenceThreshold.mockClear();
    mockAnswerResearchQuestion.mockClear();

    // Reset default mock implementation for answerResearchQuestion
  ...
  describe('Confidence Threshold Handling', () => {
    it('should use default confidence threshold of 0.6', async () => {
      ...
      expect(mockSetConfidenceThreshold).toHaveBeenCalledWith(0.6);
    });
    ...
    it('should use custom confidence threshold when provided', async () => {
      ...
      expect(mockSetConfidenceThreshold).toHaveBeenCalledWith(0.8);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should wrap orchestrator errors in McpAdrError', async () => {
      mockAnswerResearchQuestion.mockRejectedValue(new Error('Orchestrator failed'));
    ...
    it('should handle non-Error exceptions', async () => {
      mockAnswerResearchQuestion.mockRejectedValue('String error');
