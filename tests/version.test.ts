...
describe('Version Management', () => {
  ...
  test('should use generic fallback version when package.json not found', () => {
    ...
    const serverSource = readFileSync(
      join(process.cwd(), 'src', 'mcp-adr-analysis-server.ts'),
      'utf-8'
    );
    ...
    expect(serverSource).not.toContain("return '2.0.2'; // fallback version");
    expect(serverSource).not.toContain("return '2.0.15'; // fallback version");
  ...
  test('should be future-proof for version updates', () => {
    ...
    expect(serverSource).toContain("return 'unknown'");

    // Should not contain any hardcoded version numbers in fallbacks
    const fallbackVersionRegex = /return\s+'[\d.]+'\s*;\s*\/\/ fallback/;
    expect(serverSource).not.toMatch(fallbackVersionRegex);

    // Should have documentation about future-proofing
    expect(serverSource).toContain('prevents the need to update this code when version changes');
  });
});
