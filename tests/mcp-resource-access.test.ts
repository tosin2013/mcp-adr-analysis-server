...
describe('MCP Resource Access', () => {
  ...
  beforeAll(async () => {
    ...
    // Create sample ADR
    await fs.writeFile(path.join(adrDir, '001-authentication.md'), `# ADR-001: Authentication Strategy
...
`);
    ...
    test.skip('should maintain data consistency during concurrent access', async () => {
      ...
      for (const intentId of intentIds) {
        await kgManager.addToolExecution(
          intentId,
          'concurrent_tool',
          { index: intentIds.indexOf(intentId) },
          { processed: true },
          true,
          [`task-${intentIds.indexOf(intentId)}`],
          []
        );
      }
  ...
  describe('LLM Usage Patterns', () => {
    test.skip('should support LLM decision-making with complete context', async () => {
