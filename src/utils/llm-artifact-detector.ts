...
export const LLM_ARTIFACT_PATTERNS: LLMArtifactPattern[] = [
  // Debug Scripts
  {
    ...
    contentPattern:
      /print\s*\(\s*["'].*(?:debug|test|trace).*["']\s*\)|console\.log\s*\(\s*["'].*debug.*["']\s*\)|logging\.debug|debugger\s*;/i,
  ...
  // Configuration Experiments
  {
    ...
    severity: 'warning',
    filePattern:
      /^test_.*\.config\.(js|json|yaml|yml)$|^experiment_.*\.config\.(js|json|yaml|yml)$/,
...
export function detectLLMArtifacts(
  ...
  const allowedInCurrentLocation =
    matches.length === 0 ||
    matches.every(match =>
      match.pattern.locationExceptions.some(
        exception => dirPath.includes(exception) || dirPath.endsWith(exception.replace('/', ''))
      )
    );
  ...
  if (allowedInCurrentLocation) {
    ...
    matches.forEach(match => {
      match.suggestions.forEach(suggestion => {
        if (
          suggestion.includes('Move') ||
          suggestion.includes('scripts/') ||
          suggestion.includes('tests/') ||
          suggestion.includes('tools/')
        ) {
 ...
 */
export function getLLMArtifactSummary(results: LLMArtifactResult[]): {
  totalFiles: number;
