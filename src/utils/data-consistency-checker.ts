...
export class DataConsistencyChecker {
      ...
      for (const taskId of section.tasks) {
        ...
        if (!tasksInObject.has(taskId)) {
          ...
          if (options.autoFix) {
            ...
            result.fixedIssues.push(`Removed orphaned task reference ${taskId} from section ${section.id}`);
          }
        }
      }
    }
  ...
  private static async checkMetadataConsistency(
    ...
    if (data.metadata.totalTasks !== actualTotal) {
      ...
      if (options.autoFix) {
        ...
        result.fixedIssues.push(`Updated metadata totalTasks from ${data.metadata.totalTasks} to ${actualTotal}`);
      }
    }
    ...
    if (data.metadata.completedTasks !== actualCompleted) {
      ...
      if (options.autoFix) {
        ...
        result.fixedIssues.push(`Updated metadata completedTasks from ${data.metadata.completedTasks} to ${actualCompleted}`);
      }
    }
  }
  ...
  static async quickCheck(data: TodoJsonData): Promise<boolean> {
    try {
      ...
      const actualTotal = Object.keys(data.tasks).length;
      if (Math.abs(data.metadata.totalTasks - actualTotal) > 1) {
        return false;
      }
