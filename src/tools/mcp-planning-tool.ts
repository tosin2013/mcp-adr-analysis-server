...
const CreateProjectSchema = z.object({
  ...
  team: z.array(z.object({
    ...
    skills: z.array(z.string()).default([]),
    capacity: z.string()
  })).default([]).describe('Team structure'),
...
const ManagePhasesSchema = z.object({
  ...
  projectPath: z.string().describe('Project root path'),
  action: z.enum(['list', 'create', 'update', 'delete', 'transition']).describe('Phase management action'),
  ...
  }).optional().describe('Phase data for create/update'),
  targetStatus: z.enum(['planning', 'active', 'completed', 'blocked', 'cancelled']).optional().describe('Target status for transition')
});

const TrackProgressSchema = z.object({
  operation: z.literal('track_progress'),
  projectPath: z.string().describe('Project root path'),
  reportType: z.enum(['summary', 'detailed', 'gantt', 'milestones', 'risks']).default('summary').describe('Type of progress report'),
  timeframe: z.enum(['current', 'weekly', 'monthly', 'quarterly']).default('current').describe('Time frame for progress tracking'),
...
const ManageResourcesSchema = z.object({
  ...
  projectPath: z.string().describe('Project root path'),
  action: z.enum(['list', 'add', 'update', 'remove', 'allocate', 'optimize']).describe('Resource management action'),
  ...
  }).optional().describe('Team member data'),
  allocationData: z.object({
    phaseId: z.string(),
    allocation: z.number().min(0).max(100)
  }).optional().describe('Resource allocation data')
});

const RiskAnalysisSchema = z.object({
  operation: z.literal('risk_analysis'),
  projectPath: z.string().describe('Project root path'),
  analysisType: z.enum(['automated', 'manual', 'comprehensive']).default('comprehensive').describe('Type of risk analysis'),
...
const GenerateReportsSchema = z.object({
  ...
  projectPath: z.string().describe('Project root path'),
  reportType: z.enum(['executive', 'status', 'health', 'team_performance', 'milestone_tracking']).describe('Type of report to generate'),
  ...
  timeframe: z.enum(['week', 'month', 'quarter', 'project']).default('month').describe('Report time frame')
});
 ...
 */
async function loadPlanningData(projectPath: string): Promise<z.infer<typeof ProjectPlanSchema> | null> {
 ...
 */
async function savePlanningData(projectPath: string, data: z.infer<typeof ProjectPlanSchema>): Promise<void> {
...
async function createProject(args: z.infer<typeof CreateProjectSchema>): Promise<any> {
  const { projectPath, projectName, description, phases, team, importFromAdrs, importFromTodos } = args;
  
  // Check if project already exists
  const existing = await loadPlanningData(projectPath);
  if (existing) {
    throw new McpAdrError(`Project already exists at ${projectPath}. Use manage_phases to modify existing project.`, 'PROJECT_EXISTS');
  }
  ...
  if (importFromAdrs) {
    try {
      ...
      const discoveryResult = await discoverAdrsInDirectory(join(projectPath, 'docs/adrs'), true, projectPath);
      
      // Link relevant ADRs to phases
      for (const phase of projectPhases) {
        const relatedAdrs = discoveryResult.adrs.filter((adr: any) => 
  ...
  if (importFromTodos) {
    try {
      ...
      if (todoData && todoData.tasks && Object.values(todoData.tasks).length > 0) {
        ...
        for (const phase of projectPhases) {
          const relatedTasks = tasksArray.filter((task: any) => 
...
async function managePhases(args: z.infer<typeof ManagePhasesSchema>): Promise<any> {
  ...
  if (!projectPlan) {
    throw new McpAdrError(`No project found at ${projectPath}. Use create_project first.`, 'PROJECT_NOT_FOUND');
  }
...
`
    ...
    case 'transition':
      if (!phaseId || !targetStatus) {
        throw new McpAdrError('Phase ID and target status are required for transition', 'INVALID_INPUT');
      }
...
async function trackProgress(args: z.infer<typeof TrackProgressSchema>): Promise<any> {
  ...
  const overallProgress = totalPhases > 0 ? Math.round(
...
${blockedPhases > 0 ? 
  `\n⚠️ **Blocked Phases**: ${projectPlan.phases.filter(p => p.status === 'blocked').map(p => p.name).join(', ')}` : 
  ''
}
`
        }]
...
${phase.resources.length > 0 ? 
  phase.resources.map(resource => `- ${resource.role}: ${resource.allocation}% (${resource.capacity})`).join('\n') : 
...
async function riskAnalysis(args: z.infer<typeof RiskAnalysisSchema>): Promise<any> {
  const { projectPath, includeAdrRisks, includeDependencyRisks, includeResourceRisks, generateMitigation } = args;
  ...
  if (includeAdrRisks) {
    for (const phase of projectPlan.phases) {
      ...
      if (phase.linkedAdrs.length > 5) {
        ...
        if (generateMitigation) {
          riskObj.mitigation = 'Consider breaking this phase into smaller, more manageable sub-phases';
        }
        risks.push(riskObj);
      }
    }
  ...
  if (includeDependencyRisks) {
    for (const phase of projectPlan.phases) {
      if (phase.dependencies.length > 3) {
        ...
        if (generateMitigation) {
          riskObj.mitigation = 'Review and simplify dependencies, create detailed coordination plan';
        }
        risks.push(riskObj);
      }
      
      // Check for circular dependencies (simplified check)
      const dependentPhases = projectPlan.phases.filter(p => 
        p.dependencies.includes(phase.id) && phase.dependencies.includes(p.id)
      );
      ...
      if (dependentPhases.length > 0) {
        ...
        if (generateMitigation) {
          riskObj.mitigation = 'Review and restructure phase dependencies to eliminate circular references';
        }
        risks.push(riskObj);
      }
    }
...
### Next Milestones
${projectPlan.milestones.filter(m => m.status === 'pending').slice(0, 3).map(milestone => 
...
function calculateTeamUtilization(projectPlan: z.infer<typeof ProjectPlanSchema>): number {
  ...
  return Math.round(
    projectPlan.team.reduce((sum, member) => sum + member.currentWorkload, 0) / projectPlan.team.length
  );
}

function calculateProjectRisk(projectPlan: z.infer<typeof ProjectPlanSchema>): string {
  const blockedPhases = projectPlan.phases.filter(p => p.status === 'blocked').length;
  const highRiskPhases = projectPlan.phases.filter(p => p.riskLevel === 'high' || p.riskLevel === 'critical').length;
...
export async function mcpPlanning(args: any): Promise<any> {
  try {
    ...
    switch (validatedArgs.operation) {
      ...
      default:
        throw new McpAdrError(`Unknown operation: ${(validatedArgs as any).operation}`, 'UNKNOWN_OPERATION');
    }
  } catch (error) {
    if (error instanceof z.ZodError) {
      throw new McpAdrError(`Validation error: ${error.errors.map(e => e.message).join(', ')}`, 'VALIDATION_ERROR');
    }
    throw error;
  }
}