...
const RecalculateScoresSchema = z.object({
  ...
  components: z
    ...
    .default(['all'])
    .describe('Score components to recalculate'),
  forceUpdate: z.boolean().default(false).describe('Force update even if data is fresh'),
  updateSources: z
    .boolean()
    .default(true)
    .describe('Trigger source tool updates before recalculating'),
});
...
const SyncScoresSchema = z.object({
  ...
  triggerTools: z
    ...
    .optional()
    .describe('Tools to trigger for fresh data'),
...
const OptimizeWeightsSchema = z.object({
  ...
  analysisMode: z
    ...
    .describe('Method for weight optimization'),
  customWeights: z
    .object({
    ...
    .optional()
    .describe('Custom weight overrides'),
...
const ResetScoresSchema = z.object({
  ...
  component: z
    ...
    .default('all')
    .describe('Score component to reset'),
 ...
 */
async function triggerSourceToolUpdates(
  projectPath: string,
  tools: string[]
      ...
      switch (tool) {
        ...
        case 'compare_adr_progress':
          ...
          results[tool] = {
            ...
            message: 'compare_adr_progress integration pending',
          };
          break;
        ...
        case 'analyze_content_security':
          ...
          results[tool] = {
            status: 'tool_not_available',
            message: 'analyze_content_security integration pending',
          };
          break;
        ...
        case 'validate_rules':
          ...
          results[tool] = {
            status: 'tool_not_available',
            message: 'validate_rules integration pending',
          };
          break;
...
${Object.entries(sourceResults)
  .map(([tool, result]: [string, any]) => `- **${tool}**: ${result.status || 'success'}`)
  .join('\n')}
...
*All TODO.md files will automatically display updated health scores on next access.*`,
      ...
      case 'sync_scores': {
        ...
        if (todoAnalysis?.content?.[0]?.text) {
          ...
          const completionMatch = analysisText.match(/(\d+)\/(\d+) tasks? completed \((\d+)%\)/);
          const criticalMatch = analysisText.match(
            /(\d+) critical\/high priority tasks? remaining/
          );
...
${Object.entries(triggerResults)
  .map(
    ([tool, result]: [string, any]) =>
...
${Object.entries(weightOptimization.optimal)
  .map(
    ([key, value]) =>
      ...
      case 'diagnose_scores': {
        ...
        const componentAnalysis = [
          ...
          { name: 'Deployment Readiness', score: currentScores.deploymentReadiness, threshold: 75 },
          {
            ...
            score: currentScores.architectureCompliance,
            threshold: 60,
          },
...
*Score trends track the impact of human intents on project health over time.*`,
      ...
      case 'get_intent_scores': {
        ...
        if (!validatedArgs.intentId) {
          throw new McpAdrError(
            'intentId is required for get_intent_scores operation',
            'INVALID_INPUT'
          );
        }
...
## Component Scores:
${Object.entries(intentTrends.componentTrends)
  .map(
    ([component, score]) =>
...
*Intent scores track how specific human requests impact project health metrics.*`,
      ...
      default:
        throw new McpAdrError(
          `Unknown smart score operation: ${(validatedArgs as any).operation}`,
          'INVALID_INPUT'
        );
    }
  } catch (error) {
